...
public class HordeTickProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity.getType().is(TagKey.create(Registry.ENTITY_TYPE_REGISTRY, new ResourceLocation("forge:hordes")))) {
			skipthistick = false;
			if (!(entity instanceof LivingEntity _livEnt ? _livEnt.isBaby() : false) && entity.getType().is(TagKey.create(Registry.ENTITY_TYPE_REGISTRY, new ResourceLocation("forge:horde_grave_robbers")))) {
			...
			if (!skipthistick && world.getLevelData().getGameRules().getBoolean(HordeHoardModGameRules.HORDESTACKING)) {
				{
					...
					List<Entity> _entfound = world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(0.45 / 2d), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center)))
